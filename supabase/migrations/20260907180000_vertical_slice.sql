...
$$;
create or replace function public.same_school(target_school uuid) returns boolean language sql stable security definer set search_path = public as $$
...
create policy "assigned or linked class" on public.classes for select using (
  ...
  exists(select 1 from public.students s join public.parent_student_links l on l.student_id=s.id where s.class_id=id and l.parent_id=auth.uid() and l.status='active')
);
create policy "own assignment" on public.teacher_class_assignments for select using (teacher_id=auth.uid() or ((select role from public.current_profile())='principal' and exists(select 1 from public.classes c where c.id=class_id and public.same_school(c.school_id))));
...
create policy "own parent links" on public.parent_student_links for select using (parent_id=auth.uid() or ((select role from public.current_profile())='principal' and exists(select 1 from public.students s where s.id=student_id and public.same_school(s.school_id))));
create policy "authorized updates" on public.student_updates for select using (
  ((select role from public.current_profile())='principal' and public.same_school(school_id)) or teacher_id=auth.uid() or
...
create policy "authorized acknowledgement read" on public.acknowledgements for select using (
  parent_id=auth.uid() or exists(select 1 from public.student_updates u where u.id=update_id and (u.teacher_id=auth.uid() or ((select role from public.current_profile())='principal' and public.same_school(u.school_id))))
);
...
returns public.student_updates language plpgsql security definer set search_path=public as $$
declare actor public.profiles; result public.student_updates;
begin
  select * into actor from public.current_profile();
  if actor.role <> 'teacher' or not exists(select 1 from teacher_class_assignments where teacher_id=auth.uid() and class_id=p_class_id and ended_at is null) then raise exception 'not authorized'; end if;
  if not exists(select 1 from students where id=p_student_id and class_id=p_class_id and school_id=actor.school_id and active) then raise exception 'student not in assigned class'; end if;
  insert into student_updates(school_id,class_id,student_id,teacher_id,category,title,message,importance) values(actor.school_id,p_class_id,p_student_id,auth.uid(),p_category,p_title,p_message,p_importance) returning * into result;
  insert into audit_events(school_id,actor_id,event_type,target_type,target_id) values(actor.school_id,auth.uid(),'student_update_sent','student_update',result.id);
  return result;
end $$;
create or replace function public.acknowledge_update(p_update_id uuid) returns public.acknowledgements language plpgsql security definer set search_path=public as $$
...
begin
  ...
  select u.school_id into update_school from student_updates u join parent_student_links l on l.student_id=u.student_id where u.id=p_update_id and l.parent_id=auth.uid() and l.status='active';
  if update_school is null then raise exception 'not authorized'; end if;
  insert into acknowledgements(update_id,parent_id) values(p_update_id,auth.uid()) on conflict(update_id,parent_id) do update set acknowledged_at=acknowledgements.acknowledged_at returning * into result;
  insert into audit_events(school_id,actor_id,event_type,target_type,target_id) values(update_school,auth.uid(),'student_update_acknowledged','student_update',p_update_id);
  return result;
end $$;
