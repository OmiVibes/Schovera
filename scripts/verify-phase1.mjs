...
async function cleanup() {
  ...
  if (temporary.studentIds.length) {
    await admin.from('parent_student_links').delete().in('student_id', temporary.studentIds);
  ...
  for (const authUserId of temporary.authUserIds) await admin.auth.admin.deleteUser(authUserId);
...
try {
  ...
  const seedProfiles = await must(admin.from('profiles').select('id,role,email').eq('school_id', school.id));
  const profileFor = (role) => seedProfiles.find((profile) => profile.role === role);
  const teacherProfile = profileFor('teacher');
  const parentProfile = profileFor('parent');
  const principalProfile = profileFor('principal');
  expect(teacherProfile && parentProfile && principalProfile, 'Expected demo Teacher, Parent, and Principal profiles are missing.');
  ...
  const unassignedClass = await must(admin.from('classes').insert({ school_id: school.id, grade: '7', division: `V${suffix.slice(0, 5)}`, academic_year: '2026-27' }).select('id').single());
  temporary.classIds.push(unassignedClass.id);
  const unlinkedStudent = await must(admin.from('students').insert({ school_id: school.id, class_id: unassignedClass.id, roll_number: `V${suffix}`, full_name: 'Verification Child' }).select('id').single());
  temporary.studentIds.push(unlinkedStudent.id);
  const otherParentAuth = await admin.auth.admin.createUser({ email: `parent-${suffix}@verification.invalid`, password: randomUUID(), email_confirm: true });
  if (otherParentAuth.error) throw otherParentAuth.error;
  temporary.authUserIds.push(otherParentAuth.data.user.id);
  await must(admin.from('profiles').insert({ id: otherParentAuth.data.user.id, school_id: school.id, role: 'parent', full_name: 'Verification Parent', email: `parent-${suffix}@verification.invalid` }));
  await must(admin.from('parent_student_links').insert({ parent_id: otherParentAuth.data.user.id, student_id: unlinkedStudent.id, relationship_label: 'Parent', status: 'active' }));

  const otherSchool = await must(admin.from('schools').insert({ name: `Verification School ${suffix}`, code: `VERIFY-${suffix}` }).select('id').single());
  temporary.schoolIds.push(otherSchool.id);
  const otherClass = await must(admin.from('classes').insert({ school_id: otherSchool.id, grade: '7', division: 'B', academic_year: '2026-27' }).select('id').single());
  temporary.classIds.push(otherClass.id);
  const otherStudent = await must(admin.from('students').insert({ school_id: otherSchool.id, class_id: otherClass.id, roll_number: '01', full_name: 'Other School Child' }).select('id').single());
  temporary.studentIds.push(otherStudent.id);

  const parentLeak = await must(parent.from('students').select('id').eq('id', unlinkedStudent.id));
  expect(parentLeak.length === 0, 'Parent isolation failed: Parent A could read Parent B child.');
  const parentLinkLeak = await must(parent.from('parent_student_links').select('id').eq('student_id', unlinkedStudent.id));
  expect(parentLinkLeak.length === 0, 'Parent isolation failed: Parent A could read Parent B link.');
  const teacherLeak = await must(teacher.from('students').select('id').eq('id', unlinkedStudent.id));
  expect(teacherLeak.length === 0, 'Teacher assignment isolation failed: unassigned student was visible.');
  await expectRpcDenied(teacher, { p_class_id: unassignedClass.id, p_student_id: unlinkedStudent.id, p_category: 'general', p_title: 'Denied update', p_message: 'This must never be saved.', p_importance: 'normal' });
  const principalCrossSchool = await must(principal.from('students').select('id').eq('id', otherStudent.id));
  ...
  const { error: repeatedAcknowledgementError } = await parent.rpc('acknowledge_update', { p_update_id: importantUpdateId });
  if (repeatedAcknowledgementError) throw repeatedAcknowledgementError;
  const acknowledgements = await must(admin.from('acknowledgements').select('id').eq('update_id', importantUpdateId));
  ...
  const normalAcknowledgements = await must(admin.from('acknowledgements').select('id').eq('update_id', normalUpdateId));
  expect(normalAcknowledgements.length === 0, 'Normal update unexpectedly required or created an acknowledgement.');

  console.log('Phase 1 integration verification passed: persistence, Realtime, acknowledgement, roles, tenant isolation, and normal-update behaviour.');
} finally {
  await cleanup();
}
